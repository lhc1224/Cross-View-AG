...
class Model(nn.Module):
    def __init__(self, block, layers, args, num_classes=36,
        ...
        self.Hamburger = HamburgerV1(in_c=2048,n=self.n,D=self.D)
    ...
    def forward(self, exocentric, egocentric_image, label=None):
        ...
        exocentric_branch=torch.mean(exocentric_branch_conv6.view(-1,n,x_c,x_h,x_w),dim=1)

        egocentric_branch = self.conv6(pro_ego_x)

        self.exo_feature_map = exocentric_branch_conv6
        ...
        return self.exo_score, self.ego_score, exocentric_branch, egocentric_branch
    ...
    def get_fused_cam(self, target=None):
        ...
        #    target = self.score.topk(1, 1, True, True) ## torch.topk沿dim返回前k个最大值和位置（和max一样）
    ...
    def get_loss(self, gt_label, exo_branch, ego_branch,T=1.0, separate=False):  ## target是gt_label

        loss_cls = self.cls_w * (self.criterion(self.ego_score, gt_label)
                                 + self.criterion(self.exo_score, gt_label))
        ...
        loss_dist = self.distil_w * (((exo_pool - ego_pool) ** 2).sum(1)).mean(0)
        ...
        return loss, loss_cls, loss_dist
...
def MODEL(args, num_classes=36,
           align_w=0.2, distil_w=0.1,
          cls_w=1, pretrained=True,n=3,D=512):
    model = Model(Bottleneck, [3, 4, 6, 3], args, num_classes=num_classes, align_w=align_w, distil_w=distil_w,
                       cls_w=cls_w, pretrained=pretrained,n=n,D=D)
    if pretrained:
