...
class HamburgerV1(nn.Module):
    def __init__(self, in_c, n=3, D=256, R=64, args=None):
        super().__init__()
    ...
    def forward(self, x, x_2):
        ...
        tmp_x_2 = self.shortcut(x_2)
        x_2 = self.lower_bread(x_2)
        x, x_2 = self.ham(x, x_2)
        x = x.contiguous().view(-1, x_c, h, w)

        x = self.upper_bread(x)
        x_2 = self.upper_bread(x_2)
        x = F.relu(x + shortcut)
        x_2 = F.relu(x_2 + tmp_x_2)
